...
- 背景色は`#ffffff`を指定すること
- 左上に投稿者名と投稿した時刻を表示させること（時刻は`2019-07-09 10:00 + 0900`のような表示のままで構いません）
- 右上に編集リンクを表示させること
- 文章と画像が表示されていること
- 文章と画像が枠内に収まっていること
...
1. GitHubにリモートリポジトリを作成する（リポジトリ名は任意とする）
2. 作成したリポジトリに、開発したアプリケーションをpushする
3. 開発したアプリケーションをHerokuにデプロイする
4. [課題提出ページ](https://diver.diveintocode.jp/submissions/new)から、HerokuのドメインURLとGithubのリポジトリURLを提出する
